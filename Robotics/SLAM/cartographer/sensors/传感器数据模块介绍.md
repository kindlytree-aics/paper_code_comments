...
```mermaid
graph TD;
    A[原始传感器数据 (来自 ROS/其他)] --> B(SensorCollator);
    B --> C{CollatedTrajectoryBuilder (for Trajectory ID N)};
    C -- AddSensorData(collated_data) --> D(wrapped_trajectory_builder);
    D -- Pass Sensor Data --> E(LocalTrajectoryBuilder);
    E -- Process Data --> F{Local SLAM Results (Nodes, Submaps, Constraints)};
    F -- via callback/return --> D;
    D -- AddNode/AddSubmap/AddConstraint --> G(PoseGraph (Shared Global Backend));
    G -- Periodically/On Trigger --> H(Perform Loop Closure & Global Optimization);
    H -- Update Poses --> G;
```